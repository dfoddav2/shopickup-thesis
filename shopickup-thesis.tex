...
This thesis project, commonly referred to as Shopickup for the practical implementation, comes into play, to solve a major pain point for SMEs in the e-commerce space: the integration and management of shipping carriers.
...
\begin{lstlisting}[language=Python,caption={Carrier adapter implementations},label=lst:carrier-adapter-interface]
...
\begin{lstlisting}[language=Python,caption={Carrier adapter implementations},label=lst:carrier-adapter-implementations]
...
\begin{lstlisting}[language=Python,caption={Carrier dispatch example},label=lst:carrier-dispatch-example]
...
Given the project scope and the desire for a balance between extensibility and operational effort, lightness, the API gateway-centric architecture combined with adapter-based carrier integrations is selected as the most suitable trade-off and the answer to our first main research question (MRQ1).
...
For a small-sized, focused application like Shopickup, strictly adhering to these “Big Data” technologies would introduce unnecessary operational overhead, which is a common issue noted by critics, such as Krep who points out that maintaining two parallel codebases (one for batch, one for streaming) for the same purposes violates the “Don't Repeat Yourself” (DRY) principle and increases maintenance costs \cite{krepsQuestioningLambdaArchitecture2014}.
...
\begin{itemize}[itemsep=3pt]
  \item \textbf{Batch Layer}: can be implemented effectively using standard relational database aggregations or scheduled cron jobs that run during off-peak hours to generate static reporting tables, which later could be replaced by a more robust ETL pipeline if needed.
  \item \textbf{Speed Layer}: does not require a complex event-streaming engine (like Kafka) if the data volume does not demand it. Instead, frequent polling of carrier APIs (e.g., every 5 minutes, depending on the provider's limits) or lightweight webhook handlers if available can serve the same functional purpose: capturing the latest state to alert merchants of immediate issues.
...
Based on the literature research this hybrid approach seems to be a suitable candidate for our project, which I will further explore in the corresponding subsection of the methodology. %TODO: Link to the section
Were the scope of the project larger, Lambda's alternative, the Kappa architecture proposed by Kreps \cite{krepsQuestioningLambdaArchitecture2014} or even a CQRS + Event Sourcing approach \cite{pachecoMicroservicePatternsBest2018} might be considered better.
...
We will in the methodology attempt to implement some of these KPIs into our own project's dashboard according to SMART criteria \cite{ishakSMARTKPIManagement2019}, then evaluate their effectiveness based on merchant feedback and the IS system success model by DeLone and McLean \cite{savolainenPerformanceDashboardEffectiveness2023}. % TODO: Here I took this idea from a master's thesis, but they themselves are pointing to DeLone and McLean's model, so rework this part
% TODO: Link to section here
...
With this chapter's layout, I am following the natural order of the design and development process of the project, throughout which I first created the practical implementation of the carrier integration system based on the principles discussed in the literature review.
Following that, I built a real commercial application that implements this system as a core component, and started exploring the topic of the monitoring dashboard through this application as well.

The combination of the integration layer and the commercial application serves as a proof of concept that attests the validity and feasibility of the literature review's proposed architecture and answer of the first research question.
A central part of this proof of concept is the dashboard, for which most of the feasible KPIs and design choices were implemented, then further evaluated based on in-depth interviews with two merchants using the application, and based on the IS system success model by DeLone and McLean \cite{savolainenPerformanceDashboardEffectiveness2023}.
...
Through development, I found the following components to be essential for such system, which are also the ones implemented in the practical project.
...
In a practical context, the domain independent parts are referred to as the \textit{carrier integration layer} throughout this paper. It was deliberately built for Node.js,the most popular web framework and runtime in the industry\parencite{stackoverflowDeveloperSurvey2025} and was made completely available as an open-source package, which can be found on \textbf{npmjs}\parencite{shopickupNpmOrganization2026} for integrators and in raw code format on \textbf{GitHub}\parencite{shopickupIntegrationLayerGitHub2026} for developers and reviewers.
...
Multiple developers getting on board would also allow for continuous updates and improvements and relatively fast reaction to changes in the carrier APIs.
This is also one of the reasons why extensibility was a key design principle for the system.
The public repository follows a monorepo structure on GitHub, while the core package and all adapters are published separately under the \textit{shopickup} npm organization.

As of the time of writing, three shipping providers are supported for on average five of the main capabilities required by merchants, such as label creation, tracking, pickup point retrieval, etc.
The currently supported carriers are Magyar Posta Logisztika (MPL), GLS and Foxpost, which are Hungary's three largest carriers\parencite{gkidEvertekelesEsLogisztika2020}, each with their own automated postal lockers and pickup points.
Three further providers are in the process of being added, including Austrian Post, and two more regional carriers.
...
\begin{itemize}[itemsep=3pt]
  ...
  \item \textbf{Canonical models and type safety}: the whole project was built with TypeScript and is strongly typed, the core accordingly defines the canonical domain models via base types for the adapters. The adapters import these types, extend them with carrier-specific fields where required and return the output in a similar predefined format. This enforces a certain level of consistency across adapters and promotes predictability. In the following figure, we can see for example through the example of a parcel creation flow how the core defines strict types such as `Parcel`, `CreateParcelsRequest` and `CreateParcelResponse`, defining the core types end to end.
  \item \textbf{Pluggable central utilities and context}: the core also provides crucial utilities used by all adapters, such as http clients, logging, telemetry or store helpers. As of the time of writing, the http client and logging utilities are completely implemented and used. The core discipline is to keep these utilities generic and pluggable, the package does provide for example a simply initializable \textit{Axios} / \textit{Fetch} based http client, but developers also have the choice to use their own custom client as long as it adheres to or is mappable to the core's defined standard http interface type. Similarly, the logging utility provides a simple \textit{pino} logger setup, but implementers can bring their own logger instances if they can map to the standard info/debug/error level logs. The adapters then use these utilities for their internal operations.
\end{itemize}
...
On one hand, the core should be general enough to be applicable to a wide range of carriers, but as we have seen through the literature, these APIs are not uniform at all, with each carrier having their own unique requirements and quirks.
...
Perhaps, the best way to understand this is through an example, such as the `createParcel` flow mentioned before.
On the input side, the core defines a `CreateParcelRequest` type with the usual expected fields for the incoming data, such as `Parcel`.
It is apparent however, that different carriers offer unique features and services that require additional fields, such as express delivery flags, insurance options, etc.
To accommodate this, the core defines a custom field in the type of `RequestOptions`, which is deliberately left up to the adapter to define and use as they see fit.

\begin{listing}[H]
\caption{Request options extensibility in \texttt{src/core/request-options.ts}}
...
This lenient typing allows adapters to first extend the core types, then specify them to their needs, such as the following example for Foxpost's parcel creation, which extends the (aliased) core type we have seen before and specifies the `options` field with custom flags, such as `isWeb`, that indicates to the carrier whether the parcel shall be hidden in their own interface.

\begin{listing}[H]
\caption{Foxpost-specific request options in \texttt{src/adapters/foxpost/types.ts}}
...
Similarly, with the responses, the core defines a base structure for the output that includes all sensible and common fields, such as `trackingNumber` for instance in case of parcel creation, but integrators might need more details returned by the carriers.
For this reason, the core defines `rawCarrierResponse` as a field in the output, which is simply a free-form serialized version of the carrier's original response body, allowing integrators access to all details they might need without locking in to the core's defined structure.

% TODO: Diagram showing the extensibility on both ends

\subsection{Integration Layer - Carrier Adapters}
The carrier adapters are the concrete implementations of the core contract for each carrier.
...
Main design choice with the adapters was to keep them simple and focused on simply implementing the core contract in the provider's context.
...
First, it ensures that the implementation is correct, and in case of truly end-to-end testing, the tests can at any time be repeated to see whether the 

\subsection{End Application - API Gateway and Extra Components}
...
Shopickup is a multi-carrier shipping integration and monitoring platform designed for e-commerce merchants, that implements the integration layer as packages and builds around it an application that follows the principles set forth in the literature review.
...
% TODO: Here a collage of screenshots of the dashboard / pickup point picker widget / settings page etc. would be nice

Since this is a commercial application with proprietary license, unlike the integration layer itself, I will not be giving access to the source code. I will instead provide a high level overview of it, only going into greater detail for the parts relevant to the thesis's main research questions.
...
To give a better understanding of the application, I will first describe its functionality concisely in terms of user flows, then I will give an architectural overview of the system and the technologies used paying special attention to the parts relevant to the thesis.

\subsection{User Flows}
Perhaps, the best way to understand the user flows of Shopickup is through diagrams. There are two main user categories: customers and merchants, each with their own distinct flows and motivations for using the platform.
To make the flows easily readable I followed the following color scheme: blue for customer actions, yellow for choices, green for automatic system actions and purple for metadata or information.

\subsubsection{Customer Flow}
I will start with the customer flow, as it is perhaps the easiest to relate to and their motivations are simple as well: they want to receive their orders in the most convenient way possible, which is often not just home delivery, but also pickup points, which are becoming increasingly popular.
The customer flow starts once they have added items to their cart and decided to go ahead with the checkout process.
...
The reason for the first split based on navigation is due to a limitation of the Shopify platform, which does not allow widgets directly on the checkout page, leaving merchants with the only choice of setting the pickup point picker on the cart page. (Shopify Plus merchants are exempt from this limitation, which is supported by the app as well, but for the vast majority of stores this is the case.)
...
\begin{figure}[H]
  ...
  \caption{Shopickup customer flows for pickup-point selection and home delivery}
  \label{fig:shopickup-user-flows}
\end{figure}

\begin{itemize}[itemsep=3pt]
  \item \textbf{Integration Management}: Screens that are meant to allow users to easily connect their preferred providers through API keys and configure most relevant settings made available through the carriers' APIs, such as delivery times, label formats and extra services for example. This step essentially exposes each available carrier adapters' options to the user in an indirect way.
  \item \textbf{Shipping and Payment Methods Configuration}: Once there are connected and set up carriers in the system, merchants can create and connect shipping and payment methods to the shipping provider's available services. For example, the merchant can simply add a new shipping method called "Provider XYZ - Pickup Point" and connects it to the corresponding carrier, then is able to limit the payment method to for example Cash on Delivery. This allows seamless filtering of methods on the checkout page for merchant flows through Shopify's Payment and Shipping Customization APIs.
...
\begin{figure}[H]
  ...
  \caption{Shopickup merchant flows for integration, mappings, shipments, and monitoring}
...
Most of the tech stack consists of modern standard tools and frameworks that are commonly used in the industry, however in some specific regards the choices have been limited or influenced by Shopify's platform requirements for embedded apps.

\begin{itemize}[itemsep=3pt]
  \item \textbf{Backend + Frontend}: The backend + frontend is specified by Shopify to be built using \textbf{React Router v7} in the server-side rendering framework mode, which means that for small applications such as this one, both the backend and frontend needs can be fulfilled by this single server from a classical web development perspective. This all was served through \textbf{Render} with future scalability in mind. This serves the merchant flows we learned about before and hosts the dashboard as well.
  \item \textbf{Integration Layer - Backend}: The carrier integration layer is built into many of the backend endpoints, and they mainly cover the integration management and shipment management flows described earlier. These are simply imported as regular \textbf{npm} packages and directly used to communicate with the carriers' APIs without any additional API gateway or broker, as each merchant is using their own instance and credentials, there are no shared resources or rate limits to worry about.
  \item \textbf{Database}: The database was chosen to be as open as possible to avoid vendor lock-in and to allow for future flexibility via extensions. The choice ended on \textbf{PostgreSQL} hosted through \textbf{Supabase} and managed through \textbf{Prisma} as the ORM. This choice allows for a good balance between ease of use, scalability, and extensibility, as well as being a common choice in the industry.
  \item \textbf{Edge Functions}: Edge functions were an indirect requirement of the Shopify platform, as they require webhook handlers to be as fast as possible. Since \textbf{Supabase} already supports \textbf{Deno} runtime edge functions, it was a natural choice to use them for this purpose as well.
  \item \textbf{Cron Jobs}: Cron jobs are handled in a twofold way: for less critical jobs, such as cleanups we use \textbf{pg\_cron} if something can be directly handled on the database, and \textbf{node-cron} for more complex tasks. In case a task is more critical or requires longer runtimes, we delegate it to \textbf{Render Cron Jobs} to ensure consistency and reliability. (Dashboard data updates, batch analytics, package tracking, etc.)
  \item \textbf{Integration Layer - Cron}: The integration layer is also used the cron jobs for tracking each merchant's shipments and updating their status in the database, as well as updating pickup points and other relevant data from the carriers. This is done through the same npm packages as the backend, but in a different context. Since these jobs are either on a larger system level scale, or even in case of per-merchant scale could run into hundreds of requests, in this case we did implement a simple API gateway as recommended by the literature review.
  \item \textbf{Shopify Theme Extension}: The pickup point picker widget is a completely separate piece of frontend code that needs to build to WebAssembly as per Shopify's requirements. It was built using \textbf{SolidJS} and serves as an interface for customers to select their preferred delivery options during checkout. It needs to both communicate with our backend server and Shopify's APIs to function properly, and it is designed to be as lightweight and fast as possible to not overburden merchants' storefronts.
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{src/diagrams/shopickup-system-architecture-overview.png}
...
Being able to communicate with multiple providers using a unified and consistent interface could save a lot of time and effort for developers and allow them to focus on the business logic, rather than the brittle, custom one-to-one function mappings that are usually built on a per app basis.
In this case, overall development time was not shortened, as not only the provider adapters, but the core also needed to be built from scratch, but once it was time to implement the capabilities in the app, it was much more convenient and quicker.
...
\begin{itemize}[itemsep=3pt]
  ...
  \item \textbf{International carriers}: Although the carrier integration system verifiably works with multiple domestic carriers (such as MPL or Foxpost), the practical application was only released for the Hungarian market and thus does not yet support international providers.
\end{itemize}
