...
The chapter is structured accordingly, starting with the design of the carrier integration system, then giving an overview of the Shopickup application, and finally describing the design and evaluation of the monitoring dashboard.

\section{Carrier Integration Architecture Design}
As described in the literature review, after considering various architectural options, an API gateway-centric architecture combined with adapter-based carrier integrations was chosen as the most suitable trade-off for the carrier integration system.
...
https://www.npmjs.com/org/shopickup
...
Throughout the flow, we have to set and collect data passively via the user's cart metadata, which is a common way to store custom data to be later read through the event webhook sent out by Shopify and make sure that customers are only presented with the valid shipping and payment options according to their choices.
The flow ends with the customer placing the order, which then triggers Shopify to send out a webhook event that we catch and process in edge runtimes.
The customer flow is designed to be as unobtrusive and the user facing UI was built using SolidJS to be as lightweight and fast as possible in a WebAssembly buildable format.

\subsubsection{Merchant Flow}
The merchant flow is more interesting and can be divided based on the main actions that merchants can take on the platform, which are: managing integrations, managing shipments and orders, and monitoring performance through the dashboard.
I will not go into too much detail on any of these flows, but rather give a high level overview of what purpose they serve and how these flows enable merchants to achieve their goals.

\begin{itemize}[itemsep=3pt]
  \item \textbf{Integration Management}: Screens that are meant to allow users to easily connect their preferred providers through API keys and configure most relevant settings made available through the carriers' APIs, such as delivery times, label formats and extra services for example. This step essentially exposes each available carrier adapters' options to the user in an indirect way. This section is also responsible for mapping available shipping and payment methods to the different carriers, which is a crucial step for the customer flow described before.
...
% TODO: HERE DESIGN A CUSTOM DIAGRAM WITH THE MAIN SCREENS OF THE APP AS PART OF THE FLOW CHARTS
...
\begin{itemize}[itemsep=3pt]
  ...
  \item \textbf{Integration Layer - Backend}: 
  \item \textbf{Database}: 
  \item \textbf{Edge Functions}: 
  \item \textbf{Integration Layer - Edge}: 
  \item \textbf{Cron Jobs}: 
  \item \textbf{Monitoring and Analytics}: 
  \item \textbf{Shopify Theme Extension}: 
\end{itemize}

% TODO: HERE DESIGN A DIAGRAM SHOWING THE ARCHITECTURE OF THE APP AND INTEGRATION POINTS

It is apparent from the diagram that there are many different components that interact with each other inside and outside of the system's boundaries.
One of the main challenges is interactions with external systems, such as the shipping providers, because we have no control over their APIs, nor their transparency. %TODO: Is this the right expression here?
Being able to communicate with multiple providers using a unified and consistent interface could save a lot of time and effort for developers and allow them to focus on the business logic, rather than the brittle, custom one-to-one function mappings that are usually built.
